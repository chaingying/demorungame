...
export default function BadgeCard({ badge }: BadgeProps) {
  ...
  return (
    <div className="flex flex-row md:flex-col md:h-full p-4 rounded-xl border bg-card shadow-sm hover:shadow-md transition-shadow mb-2">
      {/* 左欄：圖片、名稱、ticket */}
      <div className="flex flex-col items-center justify-center w-1/3 md:w-full">
      ...
      {/* 右欄：描述、note、按鈕 */}
      <div className="flex flex-col justify-between w-2/3 md:w-full md:flex-1 md:mt-2 pl-4 md:pl-0">
        <div>
          ...
          <div className="md:mt-auto mt-2">
            {badge.id === 3 ? (
              ...
              <Button
                ...
                className="w-full border-emerald-600 text-emerald-600 hover:bg-emerald-600 hover:text-white"
                onClick={() => {
                  (document.getElementById("badge-mountain-modal") as HTMLDialogElement)?.showModal()
                }}
              >
                登山勳章地點
                <PointingHand className="h-4 w-4" />
              ...
              <Button
                ...
                className="w-full border-emerald-600 text-emerald-600 hover:bg-emerald-600 hover:text-white"
                onClick={() => {
                  (document.getElementById("badge-race-modal") as HTMLDialogElement)?.showModal()
                }}
              >
                指定賽事場次
                <PointingHand className="h-4 w-4" />
        ...
        >
          <div className="bg-white rounded-lg overflow-hidden relative">
            <Button
              ...
              onClick={() => {
                (document.getElementById("badge-mountain-modal") as HTMLDialogElement)?.close()
              }}
            >
              <X className="h-5 w-5" />
            </Button>
        ...
        >
          <div className="bg-white rounded-lg overflow-hidden relative">
            <Button
              ...
              className="absolute right-2 top-2 z-10"
              onClick={() => {
                (document.getElementById("badge-race-modal") as HTMLDialogElement)?.close()
              }}
            >
              <X className="h-5 w-5" />
            </Button>
